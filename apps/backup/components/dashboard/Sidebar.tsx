...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, currentUser, onLogout }) => {
    ...
    const NavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => (
        ...
        >
          <Icon className={`w-4.5 h-4.5 ${currentView === view ? 'text-brand-600 stroke-[2.5px]' : 'text-gray-400'}`} />
          {label}
        </button>
    );
        ...
        <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 fixed h-full z-20 overflow-y-auto scrollbar-hide">
            <div className="p-6 pb-0">
                <div className="flex items-center space-x-3 mb-8 cursor-pointer" onClick={() => onChangeView(ViewState.DASHBOARD_HOME)}>
                    ...
                    <span className="font-serif text-lg font-bold text-gray-900 tracking-tight">বিদ্যাচক্র প্রাক্তনী সমিতি</span>
                </div>
            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50/50">
                ...
                <button onClick={onLogout} className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors w-full mt-1">
                    <LogOut className="w-4.5 h-4.5" /> 
                    <span>Sign Out</span>
                </button>
                 <div className="flex items-center gap-3 mt-4 px-2">
                    <img src={currentUser.avatar} alt="Profile" className="w-8 h-8 rounded-full border border-gray-200 object-cover shadow-sm" />
